...
public class OBJ : MonoBehaviour
{
    ...
    //public GameObject miniModel = null;
    ...
    private void Build()
    {
        ...
        {
            par.AddComponent(typeof(MeshFilter));
            par.AddComponent(typeof(MeshRenderer));
            ms[0] = par;
        }
        ...
        ////
        //miniModel = Instantiate(par);
        //miniModel.name = "MiniReferenceModel";
        //miniModel.AddComponent(Type.GetType("GestureAction"));
        //miniModel.layer = LayerMask.NameToLayer("Interactible");
        //miniModel.AddComponent(typeof(BoxCollider));
        //miniModel.transform.position = new Vector3(1, 0, 4);
        //miniModel.transform.localScale = new Vector3(0.1f, 0.1f, 0.1f); // to avoid large mesh with large box collider, so that gaze cannot hit them.
        //// end
        //for (int i = 0; i < ms.Length; i++)
        //{
        //    // code by xiaojuan: add GestureAction.cs to the gameobject, and set the layer to "Interactible"
        //    miniModel.transform.GetChild(i).gameObject.layer = LayerMask.NameToLayer("Interactible");
        //    miniModel.transform.GetChild(i).gameObject.AddComponent(typeof(BoxCollider));
        //    // end
        //}
        ////

        // code by xiaojuan: add GestureAction.cs to the gameobject, and set the layer to "Interactible"
        par.AddComponent(Type.GetType("GestureAction"));
        par.layer = LayerMask.NameToLayer("Interactible");
        par.AddComponent(typeof(BoxCollider));
        par.transform.position = new Vector3(1, 0, 4);
        par.transform.localScale = new Vector3(0.1f, 0.1f, 0.1f); // to avoid large mesh with large box collider, so that gaze cannot hit them.
        // end
        for (int i = 0; i < ms.Length; i++)
        {
            // code by xiaojuan: add GestureAction.cs to the gameobject, and set the layer to "Interactible"
            ms[i].layer = LayerMask.NameToLayer("Interactible");
            ms[i].AddComponent(typeof(BoxCollider));
            // end
        }

        par.AddComponent(Type.GetType("BoundBoxes_BoundBox"));
    }
}
